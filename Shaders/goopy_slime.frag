// Goopy Slime — a single continuous mass of viscous goo, a lava-lamp read:
// a few big lobes smooth-min merged into one slowly oozing body, each lobe
// sagging on its own gentle cycle for the "slumping under its own weight"
// read. Three things make this genuinely different from Metaballs rather
// than a reskin of it: (1) the surface itself is perturbed by a rolling
// sine-noise bump field, so it reads as thick/lumpy goo instead of
// Metaballs' perfectly smooth spherical fusion, (2) a few thin, tapering
// strands actually droop off the underside of the mass down onto a floor
// below it -- real dripping, not just an orbiting blob -- and (3) a hard,
// tight specular highlight for a wet/glossy surface instead of Metaballs'
// softer plastic finish.
//
// Deliberately calm/lava-lamp paced -- an explicit user call after the
// bass/mid/treble-driven geometry (blob radius, blend width, surface
// bump, orbit speed, drip length) all changing every frame together read
// as jitter rather than a slow fluid motion. All of that is now fixed/
// time-driven only; the audio still shows up, but purely as
// brightness/glint (specular, fresnel, the final glow tint), never as
// shape or motion.
...
vec3 lobeCenter(int i, float react)
{
    ...
    float speed = mix(0.05, 0.1, hashG(seed)); // slow, lava-lamp pace -- not audio-driven
    ...
    float sagPeriod = mix(9.0, 15.0, hashG(seed + 17.0));
...
{
    return mix(0.75, 1.15, hashG(float(i) * 6.0 + 1.0)); // fixed -- no bass bulge
}

float massDE(vec3 p, float react)
{
    float blendK = 1.3; // fixed, on the soft/fluid side -- a cohesive lava-lamp blob, not popping merges
    float d = 1.0e5;
    ...
    // the march step size). Slow time frequencies (a fraction of a Hz,
    // not 1+) so it reads as a gentle roll across the surface rather than
    // vibrating -- fixed amplitude, not audio-modulated, the same "no
    // geometry reactivity" call as the rest of this preset.
    float bump = sin(p.x * 9.0 + uTime * 0.35) * sin(p.y * 7.0 - uTime * 0.45) * sin(p.z * 8.0 + uTime * 0.3);
    d += bump * 0.035;
    return d;
}
...
float dripStrandsDE(vec3 p, float react)
{
    ...
    for (int i = 0; i < kNumDripStrands; ++i)
    {
        ...
        float sway = sin(uTime * 0.25 + seed) * 0.15;
        float dripLen = mix(1.5, 2.2, hashG(seed + 3.0)); // fixed -- no bass stretch
        vec3 bottom = top + vec3(sway, -dripLen, cos(uTime * 0.2 + seed) * 0.1);
